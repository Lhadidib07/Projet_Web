...
class GridApi
{
    ...
    public function addGrid($data)
    {
        ...
        try {
            // ajouter data
            $sql = "INSERT INTO grids (user_id,title, description, rows_size,cols_size ,grid_data ) VALUES (:title, :description, :size, :difficulty, :user_id)";
            $req = $this->db->prepare($sql);
            $req->bindParam(':user_id', $data['user_id']);
            $req->bindParam(':title', $data['title']);
            $req->bindParam(':description', $data['description']);
            $req->bindParam(':rows_size', $data['rows_size']);
            $req->bindParam(':cols_size', $data['cols_size']);
            $req->bindParam(':grid_data', $data['grid_data']);
